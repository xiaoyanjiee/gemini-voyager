...
- 用户在真实 M365 Copilot 页面确认早期右上角最小导出 UI 可见，`Export JSON` 和 `Export Markdown` 两个按钮点击后均能正常导出文件；当前实现已替换为单 `Export` trigger + M365-only JSON / Markdown 弹窗。
- 2026-04-30 本轮真机复测：Codex 重建并重启临时 Edge profile 加载 `L:\project\dist_chrome`，打开真实 conversation `https://m365.cloud.microsoft/chat/conversation/3a9c838f-bbfd-48aa-a85f-4e9570d20ac8`；确认 `#gv-m365-export-ui-root` / style / trigger / dialog 均为 1，旧 `[data-gv-m365-export-format]` 按钮为 0，Export 弹窗可打开且含 JSON / Markdown，Escape 可关闭；timeline root/style/tooltip 均为 1、marker 为 2、chatWidth style 为 1、Voyager isolated world helpers 均存在。真机截图发现 timeline tooltip 顶部位置会贴近 Export，已修复为检测 `#gv-m365-export-ui-root` 并左移避让；复测确认 `intersectsExport === false`。
