...
- MVP 只实现 CSS/layout 逻辑：使用 `#gv-m365-chat-width-style`、`gv-m365-chat-width-enabled` 和 M365-only selectors，放宽 `chatMessageContainer...` 及其下方 `[role="article"]` 中 `fai-UserMessage` / `fai-CopilotMessage` 消息区域到保守宽度上限。
